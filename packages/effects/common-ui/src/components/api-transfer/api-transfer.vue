...
const { class: attrsClass, style: attrsStyle } = attrs;
...
const getOptions = computed(() => {
  const { labelField, keyField, numberToString } = props;
  ...
  function transformData(data: OptionsItem[]): OptionsItem[] {
    return data.map((item) => {
      ...
      return {
        ...objectOmit(item, [labelField, keyField]),
        [props.optionsLabelName]: get(item, labelField),
        [props.optionsKeyName]: numberToString ? `${key}` : key,
      };
    });
  }

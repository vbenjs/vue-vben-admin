...
<template>
  <div>
      ...
      <template #title>
        <div class="flex items-center">
          ...
          <input
            ...
            class="placeholder:text-muted-foreground ring-none w-[80%] rounded-md border border-none bg-transparent p-2 pl-0 text-sm font-normal ring-0 ring-offset-transparent outline-hidden focus-visible:ring-transparent"
          />
        </div>
      </template>
    ...
    >
      <Search
        class="group-hover:text-foreground text-muted-foreground size-4 group-hover:opacity-100"
      />
      <span
        class="group-hover:text-foreground text-muted-foreground hidden text-xs duration-300 md:block"
      >
        {{ $t('ui.widgets.search.title') }}
      </span>
      <span
        v-if="enableShortcutKey"
        class="bg-background border-foreground/60 group-hover:text-foreground text-muted-foreground relative hidden rounded-sm rounded-r-xl px-1.5 py-1 text-xs leading-none group-hover:opacity-100 md:block"
      >
        {{ isWindowsOs() ? 'Ctrl' : '⌘' }}
        <kbd>K</kbd>
      </span>

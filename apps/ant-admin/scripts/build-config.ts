...
import { GLOB_CONFIG_FILE_NAME, OUTPUT_DIR } from '@config/vite'
import { getAppConfigFileName } from '@pkg/utils'
...
function createConfig(params: CreateConfigOptions) {
  ...
  try {
    ...
    console.log(
      cyan(`✨ [${pkg.name}]`) + ` - configuration file is build successfully:`,
    )

...
interface ApprovalListPageProps {
  ...
  queryType:
    ...
    | 'my-todo'
    | 'pending';
  businessCategory?: 'expense' | 'loan' | 'refund';
  embedded?: boolean;
}

const props = withDefaults(defineProps<ApprovalListPageProps>(), {
...
const commandMode = ref<'add-sign' | 'approve' | 'remind' | 'withdraw'>(
  'approve',
);
...
const columns = [
  ...
  { title: '业务分类', dataIndex: 'businessCategory', key: 'businessCategory', width: 110 },
...
const wrapperProps = computed(() =>
  props.embedded ? {} : { title: props.title, description: props.description },
);
...
const fetchList = async (page = 1) => {
  try {
    ...
    message.error(error?.message || '加载审批工作台失败');
  } finally {
    loading.value = false;
  }
};
...
const openCommandModal = (
  mode: 'add-sign' | 'approve' | 'remind' | 'withdraw',
...
<template>
  <component :is="props.embedded ? 'div' : Page" v-bind="wrapperProps">
    <div :class="props.embedded ? '' : 'p-4'">
      <Card :bordered="false">
        ...
        >
          <template #bodyCell="{ column, record }">
            <template v-if="column.key === 'businessCategory'">
              {{ businessCategoryMap[record.businessCategory] || '-' }}
            </template>
            ...
            <template v-if="column.key === 'action'">
              ...
              <Button
                v-if="props.queryType === 'my-submit'"
                type="link"
                size="small"
              ...
              <Button
                v-if="props.queryType === 'my-todo' || props.queryType === 'pending'"
                type="link"
                size="small"
              ...
              <Button
                v-if="props.queryType === 'pending'"
                type="link"
                size="small"
                @click="openCommandModal('add-sign', record)"
              >
                加签
              </Button>
            </template>

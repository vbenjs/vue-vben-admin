...
import { projectLevel1Api } from '#/api/core/sys-manage';
...
const {
  ...
  api: projectLevel1Api,
  rowKey: 'id',
  defaultFormState,
  messages: {
    createSuccess: '新增一级项目成功',
    updateSuccess: '更新一级项目成功',
    deleteSuccess: '删除一级项目成功',
  },
});
...
const columns = [
  ...
  { title: '政采', dataIndex: 'isGovProcure', key: 'isGovProcure', width: 80 },
    ...
    <div class="p-4">
      <Card :bordered="false">
        ...
        <Table :columns="columns" :data-source="dataSource" :loading="loading" :pagination="pagination"
          @change="onTableChange" row-key="id" bordered size="middle" :scroll="{ x: 1200 }">
      ...
      <Modal v-model:open="isModalVisible" :title="formState.id ? '编辑一级项目' : '新增一级项目'"
        ...
        <Form ref="formRef" :model="formState" :label-col="{ span: 5 }" :wrapper-col="{ span: 17 }" class="mt-4">
          <div class="grid grid-cols-2 gap-x-4">
            <Form.Item label="项目编码" name="projectCode">
              <Input v-model:value="formState.projectCode" />
            </Form.Item>
            ...
            <Form.Item label="支出分类" name="expendCategory">
              <Input v-model:value="formState.expendCategory" />
            </Form.Item>
            <Form.Item label="功能分类" name="funcCategory">
              <Input v-model:value="formState.funcCategory" />
            </Form.Item>
            <Form.Item label="资金来源" name="fundSource">
              <Input v-model:value="formState.fundSource" />
            </Form.Item>
            <Form.Item label="政府采购" name="isGovProcure">
              <Select v-model:value="formState.isGovProcure">

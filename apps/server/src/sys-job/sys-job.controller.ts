...
export class SysJobController {
  ...
  @Get('list')
  async findAll(@Query('page') page: string = '1', @Query('pageSize') pageSize: string = '10', @Query('jobName') jobName?: string, @Query('jobGroup') jobGroup?: string, @Query('status') status?: string) {

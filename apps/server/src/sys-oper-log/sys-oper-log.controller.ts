...
export class SysOperLogController {
  ...
  @Get('list')
  async findAll(@Query('page') page: string = '1', @Query('pageSize') pageSize: string = '10', @Query('title') title?: string, @Query('operName') operName?: string, @Query('status') status?: string) {
    const skip = (Number(page) - 1) * Number(pageSize);
    const take = Number(pageSize);
    return this.sysOperLogService.findAll({ skip, take, title, operName, status: status ? Number(status) : undefined });
  }

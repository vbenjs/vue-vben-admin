...
export class SysApprovalProcessService {
  ...
  async findAll(params: { skip?: number; take?: number; processName?: string; status?: string }) {
  ...
  async findOne(id: number) {
    const item = await this.prisma.sysApprovalProcess.findUnique({ where: { processId: BigInt(id) } });
    return item ? { ...item, processId: item.processId.toString(), formId: item.formId?.toString() } : null;
  }

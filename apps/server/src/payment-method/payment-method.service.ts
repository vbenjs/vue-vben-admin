...
export class PaymentMethodService {
  ...
  async create(data: any, username: string) {
    const result = await this.prisma.paymentMethod.create({
      data: {
        ...
        payerId: data.payerId ? BigInt(data.payerId) : undefined,
        payerName: data.payerName || '',
        payerUnit: data.payerUnit || '',
  ...
  async findAll(
    ...
    const where = {
      ...
      ...(params.paymentType ? { paymentType: { contains: params.paymentType } } : {}),
  ...
  async remove(id: bigint) {
    const result = await this.prisma.paymentMethod.delete({ where: { id } });
    return this.serialize(result);
  }
  private serialize(item: any) {
    return { ...item, id: item.id.toString(), payerId: item.payerId?.toString() };
  }
}

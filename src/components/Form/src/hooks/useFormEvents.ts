...
import { dateItemType, handleInputNumberValue, defaultValueComponents, simpleComponents } from '../helper';
...
export function useFormEvents({
  ...
  async function resetSchema(data: Partial<FormSchema> | Partial<FormSchema>[]) {
    ...
    const hasField = updateData.every(
      (item) => simpleComponents.includes(item.component as string) || (Reflect.has(item, 'field') && item.field),
    );

    ...
    <BasicTable @register="registerTable">
      ...
      <template #bodyCell="{ record, column }">
        <template v-if="column.dataIndex === 'action'">
          <TableAction stopButtonPropagation :actions="createActions(record as Account)" />
        </template>
      </template>
    </BasicTable>
  ...
  const createActions = (record: Account) => {
    return [
      ...
      },
      {
        ...
        tooltip: '解锁账号',
        auth: 'accountManager_unlock',
        ifShow: record.accountNonLock === 'N',
        onClick: async () => {
          await unlockAccount([record.id]);
      ...
      },
      {
        ...
        popConfirm: {
          ...
          placement: 'left',
          confirm: () => {
            deleteAccount(record.id).then(() => reload());
          },
        },
        ifShow: record.sysDefault !== 'Y',
      },

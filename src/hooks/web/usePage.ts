...
export function useGo(_router?: Router) {
  ...
  function go(opt: RouteLocationRawEx = PageEnum.BASE_HOME, goType: boolean | GoType = false) {
    if (!opt) {
      return;
    }
    ...
    const isReplace = goType === true || goType === GoType.replace;
    const isAfter = GoType.after;
    ...
    if (isReplace) {
      ...
      // 当前 tab
      const currentIndex = tabStore.getCachedTabList.findIndex((item) => item === currentName);

      push(opt)
        .then(() => {
          // 新 tab（也是最后一个）
          const targetIndex = tabStore.getCachedTabList.length - 1;
          // 新 tab 在 当前 tab 的后面
